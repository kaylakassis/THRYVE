...
.ivy-w { position: fixed; inset: 0; background: #0D0E0C; color: #F3F3EE; overflow: hidden;
...
.ivy-w .mark { position: relative; width: 116px; height: 116px; border-radius: 32px; background: #CFFF50;
  display: flex; align-items: center; justify-content: center;
  box-shadow: 0 24px 60px -24px rgba(207,255,80,.5); animation: ivy-w-pop .7s cubic-bezier(.2,.8,.2,1) both; }
.ivy-w .mark path { stroke: #0B0C08; stroke-width: 11; fill: none; stroke-linecap: round; stroke-linejoin: round;
  stroke-dasharray: 90; stroke-dashoffset: 90; animation: ivy-w-draw .55s ease-out .5s forwards; }
...
.ivy-w .secondary { background: transparent; color: #F3F3EE; border: 1.5px solid #383D41; border-radius: 14px; padding: 15px; font: inherit; font-size: 16px; font-weight: 600; }
...
.ivy-w.settled .mark path { animation: none; stroke-dashoffset: 0 }
@media (prefers-reduced-motion: reduce) {
  ...
  .ivy-w .mark path { animation: none; stroke-dashoffset: 0 }
}
`;
  ...
  return (
    <div className={'ivy-w' + (settled ? ' settled' : '')} onPointerDown={() => setSettled(true)}>
      ...
      <div className="mark" aria-hidden="true">
        <svg width="64" height="64" viewBox="0 0 100 100"><path d="M27 52 L44 69 L75 35"/></svg>
      </div>
      <h1>Ivy</h1>
